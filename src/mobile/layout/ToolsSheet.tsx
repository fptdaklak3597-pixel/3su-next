/**
 * Sheet "Thêm" — 6 mục gốc 3su-v2.7.4 (openToolsMenu).
 */
...
const ITEMS: { path?: string; notes?: boolean; label: string; sub: string; icon: typeof PackagePlus }[] = [
  { path: '/nhap-hang', label: 'Nhập hàng', sub: 'Phiếu nhập kho nhanh', icon: PackagePlus },
  { path: '/bao-cao', label: 'Báo cáo', sub: 'Doanh thu, lời, xu hướng', icon: BarChart3 },
  { path: '/nha-cung-cap', label: 'Nhà cung cấp', sub: 'Nguồn hàng và công nợ', icon: Truck },
  { path: '/khach-hang', label: 'Khách hàng', sub: 'Nợ và lịch sử mua', icon: Users },
  { notes: true, label: 'Ghi chú', sub: 'Việc cần làm, ý tưởng', icon: StickyNote },
  { path: '/cai-dat', label: 'Cài đặt', sub: 'Shop, in, dữ liệu', icon: Settings },
]
    ...
    <div className="sheet-overlay" onClick={onClose}>
      <div className="sheet-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-2 mb-3">
          <input className="field-input text-sm" placeholder="Việc cần làm…" value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && add()} />
          <button className="btn-ghost" onClick={add} aria-label="Thêm"><Plus size={16} /></button>
        </div>
        <div className="flex flex-col gap-1.5 max-h-[50vh] overflow-y-auto">
          {sorted.map((n) => (
            <div key={n.id} className="list-row">
              <button className="flex-1 text-left min-w-0" onClick={() => toggleNoteDone(n.id)}>
              ...
              <button className="p-1 text-[11px]" style={{ color: 'var(--mute)' }} onClick={() => deleteNote(n.id)}>Xóa</button>
            </div>
          ))}
          {sorted.length === 0 && (
